...
SHEET_HEADERS = [
    "Row", "Subtopic Name", "Question", "Marks", "Answer", "Extra Information",
    "Table", "Rewriting", "Prompts", "Identity+ Prompt+ Query", "Scoring",
    "Paper", "Question Number", "Sub Part",
]
...
- question_text: the full question text exactly as written, including any bullet points, tables, or context given. IMPORTANT: For tick box or multiple choice questions, you MUST include ALL the answer options exactly as printed below the question. Look carefully at the page image to find these options. Do NOT include the mark count in the question text.
...
def extract_pdf(
    ...
    with pdfplumber.open(question_pdf_path) as pdf:
        ...
        for page_num, page in enumerate(pdf.pages):
            ...
            for q in page_questions:
                q["paper"] = paper_label
                q["page"] = page_num + 1
                q["figure_url"] = page_fig_url if q.get("figure_ref") and page_fig_url else ""
    ...
    for a in all_answers:
        ...
        if key not in answer_lookup:
            ...
            answer_lookup[key]["answer"] = answer_lookup[key].get("answer", "") + "\n" + a.get("answer", "")
            answer_lookup[key]["extra_info"] = answer_lookup[key].get("extra_info", "") + " " + a.get("extra_info", "")
